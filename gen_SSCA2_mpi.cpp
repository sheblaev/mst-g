...
void gen_SSCA2_graph_MPI(graph_t* g)
{
    ...
    uint32_t local_numIntraClusterEdges, local_numInterClusterEdges;
	weight_t* weights;
	...
	local_numIntraClusterEdges = edgeNum;
		...
		for (t = 1; t < dsize; t++) {
			...
			if ((j<firstVsInCluster[currCluster]) || \
				...
				for (k=0; k<1+((uint32_t) (MaxParallelEdges - 1)*  drand48()); k++) {
					if (p >  drand48()) {
						startV[edgeNum] = VERTEX_TO_GLOBAL(i);
						endV[edgeNum] = j;
						edgeNum++;	
					}	
				}	
	...
	local_numInterClusterEdges = local_numEdges - local_numIntraClusterEdges;	
	...
	for (i=0; i<local_numEdges; i++) {  
		startVertex[i] = VERTEX_TO_GLOBAL(permV[VERTEX_LOCAL(startV[i])]);
        if (VERTEX_OWNER(endV[i]) != rank) {
		    endVertex[i] = endV[i];
        } else {
       		endVertex[i] = permV[VERTEX_LOCAL(endV[i])];
        }
	}
    ...
    for (i=0; i<local_numEdges; i++) {
        int proc_id = VERTEX_OWNER(endVertex[i]);
        send_counts[proc_id]++;
    }
    ...
    for (edge_id_t i=0; i<local_numEdges; i++) {
        int proc_id = VERTEX_OWNER(startVertex[i]);
        ...
        proc_id = VERTEX_OWNER(endVertex[i]);
        offset = send_offsets_edge[proc_id] + 2*send_counts[proc_id];
    ...
    for (edge_id_t i=0; i<2*g->local_m; i=i+2) {
        degree[VERTEX_LOCAL(recv_edges[i])]++;
    }
